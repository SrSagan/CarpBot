...
class music(commands.Cog):
    ...
    @commands.command(
        pass_context=True,  # a command for the one no longer with us to remember her as she will always be in my heart
    ...
    async def estrelheinhas(self, ctx):
        ...
        if voice_client != None:
            ...
            if (
                vc.is_playing() == False
            ):  # si no esta reproduciendo comienza a reproducir (no hace falta pero weno)
                await b.play(vc, ctx, self.bot)
    ...
    @commands.command(
        pass_context=True,  # reproduce musica
    ...
    async def resume(self, ctx):
        ...
        if sm.exists(id):
            ...
            if vc.is_paused() == True and status == True:
                ...
                x = time.strptime(playlist.time.split(",")[0], "%H:%M:%S")
                tiempo = datetime.timedelta(
                    hours=x.tm_hour, minutes=x.tm_min, seconds=x.tm_sec
                ).total_seconds()

                x = time.strptime(playlist.ptime.split(",")[0], "%H:%M:%S")
                ptime = datetime.timedelta(
                    hours=x.tm_hour, minutes=x.tm_min, seconds=x.tm_sec
                ).total_seconds()

                time_paused = resume_time - ptime
                playlist.time = time.strftime(
                    "%H:%M:%S", time.gmtime(tiempo + time_paused)
                )
        ...
        if sm.exists(id):
            ...
            if playlist.cplaying != -1:
                ...
                time_left = time.strftime(
                    "%H:%M:%S", time.gmtime(length - time_elapsed)
                )

                # lo conviernet
                # amigo alto bardo hacer la pija esta
                embed = discord.Embed(
                    title=leng.ar[a.get_lenguaje(ctx.message)],
                    color=0x3498DB,
                    description=str(index) + "- " + (str(names[index - 1])),
                )
                embed.set_footer(
                    text=leng.duracion[a.get_lenguaje(ctx.message)]
                    + str(lengths[index - 1])
                )
                embed.set_footer(
            ...
            if len(args) != 0:
                if args[0].isnumeric():
                    if int(args[0]) <= len(playlist.songs):
                        embed = discord.Embed(
                            title=leng.removido[a.get_lenguaje(ctx.message)],
                            color=0x3498DB,
                            description=str(playlist.songs[int(args[0]) - 1].songs),
                        )
                        embed.set_footer(
                            text=leng.duracion[a.get_lenguaje(ctx.message)]
                            + str(playlist.songs[int(args[0]) - 1].length)
                        )
                        await ctx.send(embed=embed)
                    ...
                    if args[0] == "last":
                        embed = discord.Embed(
                            ...
                            color=0x3498DB,
                            description=str(
                                playlist.songs[len(playlist.songs) - 1].name
                            ),
                        )
                        embed.set_footer(
                            text=leng.duracion[a.get_lenguaje(ctx.message)]
                            + str(playlist.songs[len(playlist.songs) - 1].length)
                        )
                        await ctx.send(embed=embed)
                ...
                if deleted == playlist.cplaying - 1:
                    if deleted == len(
                        playlist.songs
                    ):  # si es la ultima cancion de la playlist vuelve una para atras
                        await self.back(ctx)
                    else:
                        vc.stop()  # sino reproduce la siguiente
                        playlist.cplaying = (
                            playlist.cplaying - 1
                        )  # la siguiente cambia a ser la que estabamos reproduciendo
    ...
    async def move(self, ctx, *args):
        ...
        if sm.exists(id) and status == True:
            ...
            if len(args) > 0:
                if args[0].isnumeric():
                    if int(args[0]) <= len(playlist.songs) and int(args[1]) <= len(
                        playlist.songs
                    ):
                        moving_song = playlist.songs[int(args[0]) - 1]
                        if args[1].isnumeric():
                            ...
                            embed = discord.Embed(
                                title="Song moved",
                                color=0x3498DB,
                                description=str(playlist.songs[int(args[1]) - 1].name)
                                + " moved to position "
                                + str(args[1]),
                            )
                            await ctx.send(embed=embed)
    ...
    async def loop(self, ctx, *args):
        ...
        if sm.exists(id):
            ...
            if playlist.looping == 0:
                playlist.looping = 1
                embed = discord.Embed(
                    description=leng.arlq_ca_d[a.get_lenguaje(ctx.message)][0],
                    color=0x3498DB,
                )
                await ctx.send(embed=embed)
                ...
                playlist.looping = 2
                embed = discord.Embed(
                    description=leng.arlq_ca_d[a.get_lenguaje(ctx.message)][1],
                    color=0x3498DB,
                )
                await ctx.send(embed=embed)
                ...
                playlist.looping = 0
                embed = discord.Embed(
                    description=leng.arlq_ca_d[a.get_lenguaje(ctx.message)][2],
                    color=0x3498DB,
                )
                await ctx.send(embed=embed)
    ...
    async def lyrics(self, ctx, *args):
        ...
        embed = discord.Embed(
            title="Results", color=0x3498DB, description="Select a result"
        )
        ...
        ):  # also check for previous next and cancel and inform of each one
            return 0
    ...
    @commands.command(aliases=["legacy_svp"], name="legacy_save_playlist")
    ...
    @commands.command(aliases=["legacy_lp", "legacy_ldp"], name="legacy_load_playlist")
        ...
        if len(args) != 0:
            ...
            while work == 2:
                ...
                if voice_client != None:
                    vc = (
                        ctx.voice_client
                    )  # si no esta reproduciendo, comienza a reproducir
                    if vc.is_playing() == False:
                        await b.play(vc, ctx, self.bot)
    ...
    # ---------------------------------------------------------SHOW PLAYLIST----------------------------------------------------------#
    @commands.command(aliases=["legacy_sp", "legacy_shp"], name="legacy_show_playlist")
    async def show_playlist(self, ctx, *args):
        ...
        if len(args) > 0:
            ...
            await ctx.send(
                embed=embed,
                view=f.control_checker(playlist=playlist, arg=1, looping=-1, ctx=ctx),
            )
            ...
            for playlist in playlists:
                ...
                embed = discord.Embed(
                    title="Playlists:", color=0x3498DB, description=text
                )
            await ctx.send(embed=embed)
    ...
    @commands.command(
        aliases=["legacy_rp", "legacy_rmpl"], name="legacy_remove_playlist"
    )
    ...
    @commands.command(aliases=["legacy_sr", "legacy_srch"], name="legacy_search")
                ...
                for song in playlist.songs:
                    ...
                    if name.find(arg) != -1:
                        text = (
                            text
                            + "\n**"
                            + str(playlist.songs.index(song) + 1)
                            + ")** "
                            + str(song.name)
                            + " *"
                            + leng.duracion[a.get_lenguaje(ctx.message)]
                            + ": "
                            + str(song.length)
                            + "*"
                        )
