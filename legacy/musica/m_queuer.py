...
class queuer:
    ...
    async def youtube_queuer(self, ctx, request):
        ...
        try:
            ...
            video = ydl.extract_info(
                f"ytsearch:{request}", download=False
            )[  # busca que es por nombre
                "entries"
            ][0]
        else:
        ...
        if type == "playlist":  # si es una playlist agrega cada cancion por separado
            ...
            embed = discord.Embed(
                title="Queued " + title,
                color=0x3498DB,
                description=str(len(tempSongs) + 1)
                + " "
                + leng.canciones[a.get_lenguaje(ctx.message)],
            )
            ...
            if server.status == True:
                ...
                embed.set_footer(
                    ...
                    + str(vid_length)
                    + "\n"
                    + leng.posicion[a.get_lenguaje(ctx.message)]
                    + ": "
    ...
    async def file_queuer(self, ctx, url):
        ...
        embed.set_footer(
            ...
            + leng.posicion[a.get_lenguaje(ctx.message)]
            + ": "
            + str(len(s.servers[s.servers_id.index(int(id))]["songs"]))
        )
        await ctx.send(embed=embed)
