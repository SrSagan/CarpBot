...
class musicManager:
    ...
    async def play(self, vc, ctx, bot):
        ...
        while True:  # comienza el loop de reproduccion
            if sm.exists(id):
                ...
                if (
                    server.cplaying + 1 > len(server.songs)
                    or server.status == False
                    or sm.exists(id) == False
                ):
                ...
                embed = discord.Embed(
                    ...
                    color=0x3498DB,
                    description=server.songs[server.cplaying].name,
                    url=url,
                )
                ...
                embed.set_footer(
                    text=leng.posicion[a.get_lenguaje(ctx.message)]
                    + ": "
                    + str(server.cplaying + 1)
                )
    ...
    def print_queue(self, playlist, arg, looping, ctx):
        ...
        for song in pages[Cpage]:
            if index + 1 == cplaying:
                text += (
                    ...
                    + song.name
                    + "** • *"
                    + leng.tr[a.get_lenguaje(ctx.message)]
                    + " "
                    + time_left
                    + "*\n"
                )
        ...
        if len(playlist.songs) - (Cpage + 1) * 10 > 0 and looping != -1:
            embed.add_field(
                name="Songs left",
                value=str(len(playlist.songs) - (Cpage + 1) * 10),
                inline=True,
            )
        ...
        if looping != 2 and looping != -1:
            embed.add_field(
                name="Looping",
                value=leng.arlq_ca_d[a.get_lenguaje(ctx.message)][looping],
                inline=True,
            )
    ...
    async def get_video_info(self, id, ctx, *index):
        ...
        if sm.exists(id):
            ...
            file_info += (
                "-**Size**: " + str(round(video["filesize"] / 1000000, 2)) + "MB\n"
            )
